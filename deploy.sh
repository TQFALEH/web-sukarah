...
echo "▶ (1/5) بناء الفِرموِير (waveshare-185)…"
"$PIO" run -e waveshare-185

echo "▶ (2/5) نسخ الصورة الكاملة → webapp/firmware/sukra-mini-$VER.bin"
...
m["version"] = v
m["builds"][0]["parts"][0]["path"] = f"firmware/sukra-mini-{v}.bin"
json.dump(m, open(p, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
print("   manifest -> version", v)
PY

echo "▶ (4/5) حفظ ودفع المصدر (master)…"
git add "webapp/firmware/sukra-mini-$VER.bin" webapp/manifest.json
